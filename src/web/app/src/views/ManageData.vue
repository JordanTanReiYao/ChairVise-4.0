  ...
  <el-main>
    <div v-if="isLogin">
      ...
      <div class="section" v-if="isReadyForChoosing">
        ...
        <select class="form-control" @change="changeVersion($event)" dir="ltr" v-model="versionId" v-if="tableType == 0">
            <option value="" selected disabled style="text-align: middle;" dir="ltr" >Record Version</option>
            <option v-for="record in authorkk" 
        ...
        <select class="form-control" @change="changeVersion($event)" dir="ltr" v-model="versionId" v-if="tableType == 1">
            <option value="" selected disabled style="text-align: middle;" dir="ltr" >Record Version</option>
            <option v-for="record in reviewkk" 
                :value="record.version" 
                :key="record.id" 
                style="text-align: middle;" 
                dir="ltr">{{record.version}}</option>
        ...
        <select class="form-control" @change="changeVersion($event)" dir="ltr" v-model="versionId" v-if="tableType == 2">
            <option value="" selected disabled style="text-align: middle;" dir="ltr" >Record Version</option>
            <option v-for="record in submissionkk" 
                :value="record.version" 
                :key="record.id" 
                style="text-align: middle;" 
                dir="ltr">{{record.version}}</option>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      reviewkk(){
        let reviewCount = Array.from(new Set(this.$store.state.record.ReviewRecordList
                                .map(v => new Object({version: v.version.versionId, type: v.version.recordType}))));
        function unique(arr, keyProps) {
        return Object.values(arr.reduce((uniqueMap, entry) => {
            const key = keyProps.map(k => entry[k]).join('|');
            if (!(key in uniqueMap)) uniqueMap[key] = entry;
      ...
      submissionkk(){
        let submissionCount = Array.from(new Set(this.$store.state.record.SubmissionRecordList
                                .map(v => new Object({version: v.version.versionId, type: v.version.recordType}))));
        function unique(arr, keyProps) {
        return Object.values(arr.reduce((uniqueMap, entry) => {
            const key = keyProps.map(k => entry[k]).join('|');
      ...
      formatType: {
        get: function () {
          return this.$store.state.dataManagement.data.formatType;
        },
      ...
      tableType: {
        get: function () {
          return this.$store.state.dataManagement.data.tableType;
        },
        set: function (newValue) {
      ...
      versionId: {
        get: function () {
          return this.$store.state.dataManagement.data.versionId;
        },
        set: function (newValue) {
      ...
      hasHeader: {
        get: function () {
          return this.$store.state.dataManagement.data.hasHeader;
        },
        set: function (newValue) {
      ...
      hasPredefined: {
        get: function () {
          return this.$store.state.dataManagement.data.hasPredefined;
        },
        set: function (newValue) {
      ...
      predefinedMappingId: {
        get: function () {
          return this.$store.state.dataManagement.data.predefinedMappingId;
        },
        set: function (newValue) {
      ...
      isReadyForMapping: function () {
        return this.$store.state.dataManagement.hasFileUploaded
          && this.$store.state.dataManagement.hasFormatTypeSpecified
          && this.$store.state.dataManagement.hasTableTypeSelected
          && this.$store.state.dataManagement.hasHeaderSpecified
          && this.$store.state.dataManagement.hasPredefinedSpecified
          && this.$store.state.dataManagement.hasVersionIdSpecified;
      },
      uploaded: function () {
        return this.$store.state.dataManagement.hasFileUploaded;
      },
      isReadyForUpload: function () {
        return this.$store.state.dataManagement.hasFormatTypeSpecified
         && this.$store.state.dataManagement.hasTableTypeSelected
         && this.$store.state.dataManagement.hasVersionIdSpecified;
      },
      isReadyForChoosing: function () {
        return this.$store.state.dataManagement.hasTableTypeSelected;
      },
    },
    methods: {
      ...
      fileUploadHandler: function (file) {
        ...
        if (!this.$store.state.presentation.versionList) {
          ...
          // filter by "AuthorRecord" "ReviewRecord" "SubmissionRecord"
          var verList;
          switch (this.$store.state.dataManagement.data.tableType) {
            case 0:
          ...
          this.$store.commit("setIsNewVersion", 
                            !verList.includes(this.$store.state.dataManagement.data.versionId));
        }
        ...
        if (this.$store.state.dataManagement.data.hasPredefined) {
            switch(this.$store.state.dataManagement.data.tableType) {
              case 0:
        ...
        Papa.parse(file.raw, {
          ...
          var res=result;
          var res2=res.data;
          var verId = this.$store.state.dataManagement.data.versionId;

          //author file preprocessing
          if( this.$store.state.dataManagement.data.tableType=="0" ){
              var authorres=[];
              //ACL file preprocessing //Softconf
              if(this.$store.state.dataManagement.data.formatType=="2"){
          ...
          //review file preprocessing
          else if( this.$store.state.dataManagement.data.tableType=="1" ){
              //Softconf
              if(this.$store.state.dataManagement.data.formatType=="2"){
                var reviewres=[];
              ...
              // Easy Chair
              else if(this.$store.state.dataManagement.data.formatType=="1"){
          ...
          else if( this.$store.state.dataManagement.data.tableType=="2" ){
              if(this.$store.state.dataManagement.data.formatType=="2"){
              var submissionres=[];
          ...
          if(this.$store.state.dataManagement.data.formatType=="1"){
            var tempCSV=[];
            //author
            if( this.$store.state.dataManagement.data.tableType=="0" ){
